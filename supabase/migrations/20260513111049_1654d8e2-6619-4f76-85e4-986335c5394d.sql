...
CREATE POLICY "Tout le monde voit les rôles métier"
  ON public.business_roles FOR SELECT TO authenticated USING (true);
...
INSERT INTO public.business_roles (name, color, position) VALUES
  ('Barista',  '#F0997B', 0),
  ('Accueil',  '#5BA8A0', 1),
  ('Host',     '#9B7EBD', 2),
  ('Cuisine',  '#E89BB0', 3);
...
-- Drop défauts qui dépendent de l'enum
ALTER TABLE public.invitations ALTER COLUMN business_roles DROP DEFAULT;

-- Convertir colonnes
ALTER TABLE public.shifts ALTER COLUMN business_role TYPE text USING business_role::text;
ALTER TABLE public.staffing_templates ALTER COLUMN business_role TYPE text USING business_role::text;
ALTER TABLE public.user_business_roles ALTER COLUMN role TYPE text USING role::text;
ALTER TABLE public.checklist_templates ALTER COLUMN business_role TYPE text USING business_role::text;
ALTER TABLE public.formations ALTER COLUMN required_role TYPE text USING required_role::text;
ALTER TABLE public.training_paths ALTER COLUMN required_role TYPE text USING required_role::text;
ALTER TABLE public.invitations ALTER COLUMN business_roles TYPE text[] USING business_roles::text[];

-- Restaurer le défaut en text[]
ALTER TABLE public.invitations ALTER COLUMN business_roles SET DEFAULT '{}'::text[];

DROP TYPE IF EXISTS public.business_role;
