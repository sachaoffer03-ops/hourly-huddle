CREATE OR REPLACE FUNCTION public.migrate_studios_v2(pairs jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Étape A : suppression de tous les shifts
  v_step := 'A_delete_shifts';
  WITH d AS (DELETE FROM public.shifts RETURNING 1) SELECT count(*) INTO v_shifts_deleted FROM d;

  -- Étape B : suppression des anciens staffing_templates des VRAIS studios
  ...
  -- Étapes C/D/E par paire
  FOR i IN 1..array_length(v_src_ids, 1) LOOP
    v_src := v_src_ids[i];
    v_dst := v_dst_ids[i];

    -- C : migrer les nouveaux templates du doublon vers le vrai
    v_step := format('C_move_templates[%s->%s]', v_src, v_dst);
    WITH u AS (
      UPDATE public.staffing_templates SET studio_id = v_dst WHERE studio_id = v_src RETURNING 1
    ) SELECT count(*) INTO v_tmp FROM u;
    v_templates_moved := v_templates_moved + v_tmp;

    -- D1 : dédupliquer user_studios
    ...
    WITH u AS (
      UPDATE public.user_studios SET studio_id = v_dst WHERE studio_id = v_src RETURNING 1
    ) SELECT count(*) INTO v_tmp FROM u;
    v_user_studios_moved := v_user_studios_moved + v_tmp;

    -- E : rebrancher les profils qui pointent encore le doublon
    v_step := format('E_move_profiles[%s->%s]', v_src, v_dst);
    WITH u AS (
      UPDATE public.profiles SET studio_id = v_dst WHERE studio_id = v_src RETURNING 1
    ) SELECT count(*) INTO v_tmp FROM u;
    v_profiles_moved := v_profiles_moved + v_tmp;

    -- Bonus : invitations qui référencent encore le doublon
    ...
    -- Bonus : checklist_templates / signalements
  ...
  -- Étape F : suppression des studios doublons
  v_step := 'F_delete_duplicates';
  WITH d AS (
    DELETE FROM public.studios WHERE id = ANY(v_src_ids) RETURNING 1
